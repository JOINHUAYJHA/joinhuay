...
// Schema สำหรับเก็บการตั้งค่าทุกอย่างแบบ Key-Value
...
app.post('/api/bills', async (req, res) => {
    try {
        ...
        io.emit('data_updated', { message: `📥 มีบิลใหม่เข้า` });
...
app.post('/api/admin/process-results', checkAuth, async (req, res) => {
    try {
        ...
        // ปรับสถานะในบิลและเพิ่มเครดิต (โค้ดนี้ถูกจัดการจากหน้าเว็บแล้ว ส่งแค่ให้หลังบ้านเซฟให้ลูกค้า)
        // เพื่อความง่าย จะปล่อยให้ AppData โหลดไปจัดการเองแบบเก่า
        res.json({ status: 'success', message: 'รับผลเรียบร้อย กรุณาเช็คเครดิตลูกค้า' });
...
app.post('/api/archive', checkAuth, async (req, res) => {
    // ลบหรือย้ายบิล (ในที่นี้เราสามารถลบทิ้งหรือติดป้ายได้ เพื่อความง่ายเราจะลบบิลที่จ่ายผลแล้ว)
